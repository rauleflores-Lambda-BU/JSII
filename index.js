...
function lowerCaseStrings(strings) {
  ...
  strings.forEach((arrItem) => {
    lowerCaseArray.push(arrItem.toLowerCase());
  })
...
function isItAnApple(strings) {
  let trueOrFalse = strings.map((arrItem) => {
...
function removeApple(strings) {
  let noApple = strings.filter((arrItem) => {
    return arrItem !== "apple";
  })
  return noApple;
}
...
function stringSmash(strings) {
  let oneString = strings.reduce((accumulator, currentValue, index) => {
...
function getFullNames(runners) {
  ...
  runners.forEach((arrItem) =>{
    fullNames.push(`${arrItem.last_name}, ${arrItem.first_name}`);
  })
  return fullNames;  
}
...
function firstNamesAllCaps(runners) {
  const firstNameCaps = runners.map((arrItem) => {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let sizeList = runners.filter((arrItem) => {
    arrItem.shirt_size === tShirtSize; 
  })
  return sizeList;
}
